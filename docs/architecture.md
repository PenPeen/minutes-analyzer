...
| 指標 | **Amazon Nova Lite** | **Gemini 2.5 Flash** | **GPT-4o mini** | **Claude 3.5 Haiku** | 重要度 |
|------|---------------------|---------------------|------------------|---------------------|--------|
| **MMLU** (総合知識・専門用語理解) | 59.0% | 80.9% | 82.0% | 76.7% | ⭐⭐⭐ |
| **JGLUE** (日本語言語理解) | データなし | データなし | データなし | データなし | ⭐⭐ |
...
**決定要因:**
1. **コンテキスト長の圧倒的優位性**: 100万トークンで無制限の拡張性を確保
2. **将来性重視**: 長時間会議や複数会議の一括処理にも対応可能
3. **十分な処理精度**: 他モデルと同等の議事録分析性能
4. **技術的安定性**: Googleの成熟したインフラとAPI提供

**Nova Liteとの比較検討:**
Amazon Nova Liteは76%のコスト削減（月$0.13 vs $0.54）を実現し、コストパフォーマンスでは優秀だが、コンテキスト長が30万トークンに制限される。将来の極長時間会議や複数会議同時処理を考慮し、拡張性を最重要視してGemini 2.5 Flashを選定。
